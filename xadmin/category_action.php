...
if($sl_id!='' && $action=='Active'){
	$sql1 = "update category set valid=1  where sl_id='$sl_id'";
}else if($sl_id!='' && $action=='Regular'){
	$sql1 = "update category set valid=0  where sl_id='$sl_id'";
}
else if($sl_id!='' && $action=='Featured'){
	$sql1 = "update category set featured=1  where sl_id='$sl_id'";
}
else if($sl_id!='' && $action=='Normal'){
	$sql1 = "update category set featured=0  where sl_id='$sl_id'";
}
